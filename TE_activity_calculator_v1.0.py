...
if blast==0:
    parsing=os.system("python %s\\codes\\blast_parsing___xml_format_v2.6.py -ov_len 10000000 -lim_score %s -num_hsp 1 -lim_evalue 1 -xml %s" % (current_directory, option_dict['-min_score'], option_dict['-fasta']+".xml" ))
    if parsing==0:
        activity=os.system("python %s\\codes\\TE_activity_calculation_by_UPGMA_v.1.0.py -csv %s" % (current_directory, option_dict['-fasta']+".csv" ))
